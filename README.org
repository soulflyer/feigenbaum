...
Open the file feigenbaum-experiment-root-drone.pd
...
Note that this generates midi notes, not sounds, so you will also need to connect it up to a midiplayer of some kind. That's a whole can of worms. Once you have something running that will take midi input, select it in `Pd:Preferences:Midi` Set the midi channels in the receiving program and the 2 inputs to *noteout* at the bottom of the pd patch. 
